...
class NewsController {
  ...
  public async fetchLatestNews(): Promise<void> {
    try {
      ...
      const newArticles = NewsModel.addArticles(latestArticles);
  ...
  // 기사 읽음 상태 변경
  public markArticleAsRead(articleId: string): void {
    NewsModel.markArticleAsRead(articleId);

    // 해당 기사의 알림도 읽음 상태로 변경
    const notifications =
      NotificationModel.getNotificationsByArticleId(articleId);
    notifications.forEach((notification) => {
      NotificationModel.markNotificationAsRead(notification.id);
    });
  }

  // 기사 필터링 (키워드 기반)
  public filterArticlesByKeyword(keyword: string): NewsArticle[] {
    const articles = NewsModel.getArticles();
  ...
  // 기사 상세 조회
  public getArticleDetail(articleId: string): NewsArticle | undefined {
    return NewsModel.getArticleById(articleId);
  }
}
